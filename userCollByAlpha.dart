...
String selectedItem;
class userCollByAlpha extends StatefulWidget {
  userCollByAlpha({
    Key key,
  }) : super(key: key);

  @override
  _userCollByAlphaState createState() => _userCollByAlphaState();
}

class _userCollByAlphaState extends State<userCollByAlpha> {
  ...
  void UserReminder (int index) async {
    ...
    for (int i = 0; i < logUser.sortedColl.length ; i++){
      if (logUser.userCollection.asMap()[i].plantName == logUser.sortedColl.asMap()[index].plantName){
        plantIndex = i;
        //logUser.userCollection.removeAt(i);
        break;}
    }
    await _databaseRef.orderByChild("ename").equalTo(logUser.sortedColl.asMap()[index].getPlantName()).once().then((DataSnapshot snap){
      ...
      for (var i in plantKey){
        second = new Plant(plantData[i]["arname"].toString(),
            logUser.sortedColl.asMap()[index].plantName, plantData[i]["irrigation"].toString(),
            plantData[i]["sunlight"].toString(), plantData[i]["temp"]);
        second.serial = plantData[i]["serialnum"].toString();
        second.image = plantData[i]["URL"];
        second.addedDate = logUser.sortedColl.asMap()[index].date;
      } // end for
    }
    ...
    await _databaseRef.orderByChild("ename").equalTo(logUser.sortedColl.asMap()[index].getPlantName()).once().then((DataSnapshot snap){
      ...
      for (var i in plantKey){
        second = new Plant(plantData[i]["arname"].toString(),
            ...
            plantData[i]["sunlight"].toString(), plantData[i]["temp"]);
        second.serial = plantData[i]["serialnum"].toString();
        second.image = logUser.sortedColl.asMap()[index].image;
        second.addedDate = logUser.sortedColl.asMap()[index].date;
      } // end for
    }
  ...
  void deletePlant (int index) async { //bring information of the selected plant to display it
    ...
    logUser.sortedColl.removeAt(index);
    await _databaseRef.orderByChild("email").equalTo(logUser.getEmail()).once().then((DataSnapshot snap){
      ...
      for(int i = indexInColl ; i< staticLength ; i++ ){
        ...
        _databaseRef.child(key.toString().substring(1, key.toString().length - 1)).child("pick"+i.toString()).remove();
      ...
      int staticIndex = indexInColl;
      for (int i = staticIndex+1 ; i< staticLength; i++ ){
        print(logUser.userCollection.asMap()[i].plantName);
        _databaseRef.child(key.toString().substring(1, key.toString().length - 1)).update({'ename'+(indexInColl).toString(): logUser.userCollection.asMap()[i].plantName,  'date'+(indexInColl).toString(): logUser.userCollection.asMap()[i].date , 'arname'+(indexInColl).toString(): logUser.userCollection.asMap()[i].arabicPlant , 'image'+(indexInColl).toString(): logUser.userCollection.asMap()[i].image});
        if(logUser.userCollection.asMap()[i].alarmDays != '')
          _databaseRef.child(key.toString().substring(1, key.toString().length - 1)).update({'day'+(indexInColl).toString(): logUser.userCollection.asMap()[i].alarmDays, 'picked'+(indexInColl).toString(): logUser.userCollection.asMap()[i].picked.hour.toString()+':'+logUser.userCollection.asMap()[i].picked.minute.toString() });
        indexInColl++;
      }
                ...
                itemBuilder: (BuildContext context, int index) {
                  if(logUser.userCollection.length > 0 && index < logUser.userCollection.length ) {
                    return Stack(
                      children: <Widget>[
                        InkWell(
                          child: Image.network(logUser.sortedColl.asMap()[index].getImage()),
                          onTap: () {
                            var i = index;
                            showModalBottomSheet(
                                context: context,
                        ...
                        Transform.translate(
                            ...
                            child: Container(
                              ...
                              child: Text(
                                logUser.sortedColl.asMap()[index].arabicPlant,
                                style: TextStyle(
                            ...
                            child: Column(
                              children: <Widget>[
                                ListTile(
                                    ...
                                    onTap: () async {
                                      Navigator.push(context, MaterialPageRoute(builder: (BuildContext context) => userCollByAlpha()));
